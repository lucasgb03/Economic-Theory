...
## Data
This dataset contains annual observations of CPI, Money supply (M2), Real GDP, and Nominal GDP yearly since 1959, all pulled from the Federal Reserve Economic Data, linked here: https://fred.stlouisfed.org/. 

To use my new equation, I had to get the data in log differece form. Note that the velocity of money formula is nominal GDP / Money supply. Real GDP is used as the output (Y) in this study. This created the following variables: 
...
I used R's predict function to predict the change in P, or inflation, for each year since 1959 based on the growth rates of M, Y, and V. The results are can be seen in the graph labeled "Inflationpredicted" in this repository. An interesting point can be seen around index 20 (year 1979-1980) where my model and the PY=MV equation overpredict inflation. This may be due to the increase in money supply in the late 1970s due to loose monetary policy and credit expansion. Fed chairman Paul Vlocker then rapidly hiked interest rates in 1979 up to 20%, curbing inflation immediately. Thus, while the money supply was high, inflation was curbed due to the higher interest rates before money supply was adjusted, causing the model to overpredict inflation based on the high money supply.
